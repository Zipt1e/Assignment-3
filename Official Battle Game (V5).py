...
## Add  (this part makes sure the animation stays down when dead and doesnt go back to idle)
            if self.action == 3:
                self.frame_index = len(self.animation_list[self.actio]) - 1
            else:
                self.idle()
## END add 
...
## ADD  
        target.death()
## END ADD, now go and add def death 
...
#   i have disabled these      potion_button = button.Button(screen, 100, HEIGHT - bottom_panel + 90, potion_img, 64, 64)
#   i have disabled these   restart_button = button.Button(screen, 330, 120, restart_img, 120, 30)
...
##chnage the line below, this will stop you being able to kill bandit when they are down 
    ...
    draw_text(str(knight.potions), font, red, 150, HEIGHT - bottom_panel + 90)
        ...
        # Player action 
### so add this line below but only this. before you add this libe, highlight from current_fighter = -1 which is on line 355. (might change as code increases, note placed there)
    if game_over == 0:
    ...
    #check if bandit needs to heal first
                ...
                current_fighter = 1 
#start your highlight for indetation 

    #check all bandits have had a turn 
...
if game_over !=0:
    ...
    if restart_button.draw()
        knight.reset()
        for bandit in bandit_list: # if this doesnt work then change to bandit
            bandit.reset()
            for bandit in bandit_list:
                bandit.reset()
            current_fighter = 1 
